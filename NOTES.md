...
### デザイン刷新（2026-09-19 社長決定・未着手・必ずやる）
- 理由：現行ページが文字と表だけで地味なため
- 方向性：リハビリデイサービスyawaragi のHP（https://www.keepfitlife-yawaragi.com/）と色・雰囲気を揃える（配色はやわらぎロゴから抽出）
- 写真：やわらぎHPの写真を流用し、`assets/` に保存して使う（Wixの画像URLを直接参照しない）
- 写真の除外ルール：利用者様の顔がはっきり分かる写真は使わない（掲載許可がやわらぎHP用のため）／ストック写真（散歩する老夫婦、多肉植物とクリップ）は使わない／東松山市マスコット（まっくん・あゆみん）は使わない／文字入りバナーは使わない
- 進め方：写真候補を選んだ段階で社長に一覧を見せて「写真OK」をもらってから仕上げる
- 文言（会社概要・代表挨拶・事業所・採用）は変えない。ファーストビューに一言メッセージを追加予定
- 静的HTML＋CSSのみ（JavaScriptなし）
- 実装時のモデル：デザイン品質重視のため上位モデル（クロが指示書で指定）
- 朝報告の宿題欄にも登録済み（社長専用タスク台帳）。着手時はクロが指示書を出す

### 別件メモ
- 求人サイト（レバウェル介護）の yawaragi 掲載で、HP URL が keepfitlife.com、住所が旧住所「高坂1302」のまま。現在 keepfitlife.com は法人ページに着く。住所修正は掲載元への依頼が必要（未着手）
